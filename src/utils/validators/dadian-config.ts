 ...
 * 头像配置 Schema
 */
const ConfigAvatarSchema = z.object({
 ...
 * 名片配置 Schema
 */
const ConfigNameCardSchema = z.object({
 ...
 * 字体资源配置 Schema
 */
const ConfigFontResourcesSchema = z.object({
...
const ConfigDictWithChildrenSchema = <T extends z.ZodTypeAny>(valueSchema: T) =>
  ConfigDictSchema(valueSchema).extend({
 ...
 * 图层配置 Schema
 */
const ConfigTileLayerSchema = z.object({
 ...
 */
const ConfigBoundsSchema = z.tuple([
 ...
 * 1.6 海岛版本配置 Schema
 */
const ConfigExtra16IslandSchema = z.object({
 ...
 * 2.8 海岛版本配置 Schema
 */
const ConfigExtra28IslandSchema = z.object({
 ...
 * 附加层级配置 Schema
 */
const ConfigExtraUndergroundSchema = z.object({
 ...
 */
const ConfigExtraSchema = z.object({
...
const ConfigPluginOverlayItemSchema = z.object({
  ...
  ).optional().describe('子项列表'),
}).describe('叠加图层子项配置')

/**
 * 叠加图层配置 Schema
 */
const ConfigPluginOverlayConfigSchema = z.object({
...
const ConfigPluginSchema = z.object({
  ...
  extraConfig: ConfigExtraSchema.optional()
    .describe('额外配置表（key 与 `extra` 中的 key 对应）'),
 ...
 */
export const DadianConfigSchema = z.object({
  ...
  }).optional().describe('应用配置'),
  editor: z.object({
    fontResources: z.record(z.string(), ConfigFontResourcesSchema)
      .describe('字体资源（key 样例 `HYWenHei-55S`）'),
    quickInputSnippets: z.array(z.string()).optional().describe('快速输入片段'),
    refreshTimeSpecial: z.array(ConfigDictSchema(z.number())).optional().describe('刷新时间字典列表'),
  }).optional().describe('编辑器配置'),
  tiles: z.record(z.string(), ConfigTileLayerSchema).optional().describe('公测图层配置（key 样例 `A:APPLE:1_6`）'),
  tilesNeigui: z.record(z.string(), ConfigTileLayerSchema).optional().describe('内测图层配置（key 样例 `提瓦特-base0`）'),
